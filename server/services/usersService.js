...
const USER_FIELDS = 'id, username, name, employee_id, department, role, disabled, can_assign, can_verify, created_at';
...
export async function createUser({ username, password_hash, name, employee_id, department, role, can_assign, can_verify }) {
  const { data, error } = await supabase
    .from('users')
    .insert({
      username,
